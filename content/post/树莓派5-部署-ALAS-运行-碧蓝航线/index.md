...
作为树莓派官方操作系统内置的远程桌面解决方案，RealVNC 在局域网环境下的使用最为直接和便捷。

其启用过程非常简单，仅需通过 sudo raspi-config 命令，在 Interface Options -> VNC 菜单中选择启用即可，无需额外安装。

随后，在同一局域网下的任何电脑上安装 VNC Viewer 客户端，输入树莓派的 IP 地址，即可轻松连接到其图形桌面。

### 5.2 `NoMachine`

对于非同一个局域网的远程控制，我们最初尝试了 `NoMachine` 作为远程桌面解决方案。在 `X11` 显示服务器环境下，`NoMachine` 表现出良好的性能和流畅度。然而，为了让 `Waydroid` 能够正常运行其图形界面，我们必须将树莓派的显示服务器切换回 `Wayland`。遗憾的是，`NoMachine` 在 `Wayland` 环境下并不兼容，所以舍弃。
...
### 5.4. 通过 frp 安全访问 ALAS Web 界面

`ALAS` 的图形用户界面 (GUI) 是通过 Web 页面进行访问和管理的。为了避免在资源有限的树莓派上直接运行消耗内存的浏览器，并且能够从外网安全地进行访问，我们采用了 `frp` (Fast Reverse Proxy) 方案，将树莓派上 `ALAS` 的本地 Web 服务端口映射到云服务器的一个公共端口上，同时启用了 `frp` 的加密功能，确保了从树莓派到云服务器的整条通信链路都是加密的，有效防止了管理界面在公网中被直接暴露或窃听。

通过这种方式，我们现在可以从任何地方，通过浏览器安全地访问 `ALAS` 的管理界面，而无需在树莓派本地进行任何图形操作。
...
解决问题的关键在于思路的转变。在多次尝试只通过构建复杂环境（如 `LD_PRELOAD` 依赖注入）来适应程序，但最终都不能完全解决所有问题。然后，我们通过`LD_PRELOAD` 依赖注入 + 直接修改 `ALAS` 源码的方式，让程序主动适应了当前环境。这一“反其道而行之”的策略，最终以最小的改动成本，优雅地解决了最核心的难题。
